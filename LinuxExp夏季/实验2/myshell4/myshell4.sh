...
请输入选项序号：" 
   		...
   		case $option in
            ...
            "1")
                     ...
                     add_teacher $teacherID $name $password
              ;;
            "2")
                     echo "请输入要修改的教师工号:"
                     read teacherID
...
###########################################    教师操作    ########################################################
function login_teacher() #	$1:帐号 $2:密码
{
	declare -i account=-1
	declare -i i=0
	find_teacher $1 
...
function course_student_manage(){
	declare option
	while [ "$option" != "0" ]
	do
		echo "0.返回
...
function teacher_course_manage(){
	declare option
	declare -i number=0
	while [ "$option" != "0" ]
	do
...
请输入选项序号：" option
		case $option in
			...
			"2")
				...
				result=$?
                number=$result
                first=$(echo ${course[$number]}|cut -d ' ' -f1,2)
                last=$(echo ${course[$number]}|cut -d ' ' -f1,2,3 --complement)
...
请输入选项序号：" option
		case $option in
			...
			"2")
				...
				result=$?
				first=$(echo ${homework[$result]}|cut -d ' ' -f1,2)
                last=$(echo ${homework[$result]}|cut -d ' ' -f1,2,3 --complement)
...
############################################       add / find函数    ###################################
function add_admin() #$1:name $2:passwd
{
	find_admin $1
	result=$?
	if [ $result = "255" ]
...
function add_teacher() # $1:teacherID $2:name $3:passwd
{
	teacher[${#teacher[*]}]="$1 $2"
...
function add_student() # $1:studentID $2:name $3:passwd#要处理很多数组
{
	student[${#student[*]}]="$1 $2"
...
function find_sub() # $stu_course $courseID
{
	declare -i i=0
	declare -a string
	string=($1)
...
function find_homework() # $hwnumber
{
	declare -i i=0
	declare -a string
...
function find_course() # $1:课程名 返回课程编号而不是课程号
{
	declare -ai result
	declare -i cou=0
	declare -i i=0
...
<<COMMENT
程序要求：
 	用 bash 编写程序，实现一个简单的 作业管理系统。用文件形式实现数据的存
	储。系统至少具备以下的基本功能：
	系统中根据不同的权限分为三类用户：管理员、教师、学生，简要说明如下：
管理员：
	创建、修改、删除、显示（list）教师帐号；教师帐户包括教师工号、教师
	姓名，教师用户以教师工号登录。
	创建、修改、删除课程；绑定（包括添加、删除）课程与教师用户。课程名
	称以简单的中文或英文命名。
教师：
